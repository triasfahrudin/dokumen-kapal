...
<script type="text/javascript">
	...
	<?php if(!empty($selected_tahun)){ 
		...
		foreach ($selected_tahun as $key) {
			$sql .= "IFNULL(" . $alpha[$loop] . ".rating,0) AS rating_". $alpha[$loop] .",";
			$join .= "LEFT JOIN (SELECT MONTH(tgl_mohon) AS bln,
			                     		AVG(rating_kepuasan) AS rating 
			                     FROM sertifikat_keselamatan 
			                     WHERE YEAR(tgl_mohon) = ". $key ." AND `status` IN('400') 
		...
		window.onload = function() {
			...
			window.myLine = Chart.Line(ctx, {
				...
				options: {
					...
					title: {
						display: true,
						text: 'Rata-rata Penilaian proses Sertifikat Keselamatan per tahun'
					},
					scales: {
						yAxes: [
							{
